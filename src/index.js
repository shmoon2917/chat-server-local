...
const {port, clientId, clientSecret} = require("./config");
...
app.post("/api/playlistItems", async (req, res) => {
  ...
  try {
    ...
    const hvoItemsLength = await getPlayListItems(latestHvoList.id);
...
app.get("/api/search", async (req, res) => {
  try {
    console.time("get lists by search");
    ...
    console.timeEnd("get lists by search");
